...
class FileList(CoreModel):
    ...
    cell_id = models.IntegerField(choices=CELL_ITEM_LIST, default=0, verbose_name="血细胞ID", help_text="血细胞ID", blank=True)
    cell_name = models.CharField(max_length=200, default="待识别", null=True, blank=True, verbose_name="血细胞名称", help_text="血细胞名称")
    is_confirmed = models.BooleanField(default=False, verbose_name="是否核验", help_text="是否核验")
    ...
    def save(self, *args, **kwargs):
        ...
        if self.id:
            self.cell_name = self.CELL_ITEM_LIST[self.cell_id][1]
...
class RecognitionConfig(CoreModel):
    parent = models.ForeignKey(
        to="self",
        verbose_name="父级",
        on_delete=models.CASCADE,
    ...
    placeholder = models.CharField(max_length=50, null=True, blank=True, verbose_name="提示信息", help_text="提示信息")
    setting = models.JSONField(null=True, blank=True, verbose_name="配置", help_text="配置")
    readonly = models.BooleanField(default=True, verbose_name="编辑状态", help_text="编辑状态")

    class Meta:
        db_table = table_prefix + "recognition_config"
        verbose_name = "识别界面表"
        verbose_name_plural = verbose_name
        ordering = ("sort",)
        unique_together = (("key", "parent_id"),)
